#' ---
#' title: "WEF 2017 Global Risks Survey"
#' author: ""
#' date: ""
#' output:
#'   html_document:
...
#+ include=FALSE
theme_wef <- function(x) {
  theme_hrbrmstr_msc(grid="XY") +
    theme(panel.border=element_rect(color="#b2b2b2", fill=NA, size=0.25)) +
    theme(panel.grid=element_line(linetype="dotted", size=0.75)) +
    theme(legend.position="none") +
    theme(plot.subtitle=element_text(family="MuseoSansCond-300")) +
    theme(axis.title.x=element_text(size=12, hjust=0)) +
    theme(axis.title.y=element_text(size=12, hjust=0))
}

theme_wef2 <- function(x) {
...
theme_wef3 <- function(x) {
  theme_hrbrmstr_msc(grid="XY") +
    theme(panel.border=element_rect(color="#b2b2b2", fill=NA, size=0.25)) +
    theme(panel.grid=element_line(linetype="dashed", size=2)) +
    theme(legend.position="none") +
    theme(plot.subtitle=element_text(family="MuseoSansCond-300")) +
    theme(axis.title.x=element_text(size=12, hjust=0)) +
    theme(axis.title.y=element_text(size=12, hjust=0)) +
    theme(axis.text.x=element_text(hjust=c(0, rep(0.5, 5), 1))) +
...
#+ zoom_group, fig.width=9, fig.height=7
...
#+ macro_group, fig.width=9, fig.height=7
ggplot(risk_by, aes(likelihood, impact, color=category)) +
  geom_point(shape=18, size=3) +
...
#+ zoom_opp, fig.width=9, fig.height=6
...
#+ macro_opp, fig.width=9, fig.height=6
ggplot(risk_groups, aes(x=left_group, xend=right_group, y=left_value, yend=right_value, color=category)) +
  geom_segment(size=0.25) +
  scale_x_discrete(name=NULL, expand=c(0,0)) +
